...
int main() {
    ...
    static sensor_co2_params co_sr = {};
    // Queue
   // rt0.comm_rot = xQueueCreate(20, sizeof(bool));
    //rt_sw.comm_sw = xQueueCreate(5, sizeof(bool));
    co_sr.comm_co2 = xQueueCreate(50, sizeof(uint16_t));
   // dp0.comm_rot = rt0.comm_rot;
   // dp0.comm_sw = rt_sw.comm_sw;
   // dp0.comm_co2 = co_sr.comm_co2;
    auto uart{std::make_shared<PicoOsUart>(1, 4, 5, 9600, 2)};

    CO2Sensor co2(1, 4, 5, 9600, 240, co_sr.comm_co2, uart); // UART1, TX4/RX5, 9600 bps, Modbus address 240
  ...
  //  xTaskCreate(rotary_task, "rotary_encoder", 256, (void *)&rt0,
   ...
   // vQueueAddToRegistry(dp0.comm_rot, "rotary_queue");
    ...
    //vQueueAddToRegistry(co_sr.comm_co2, "co2_sensor_queue");
    vTaskStartScheduler();
